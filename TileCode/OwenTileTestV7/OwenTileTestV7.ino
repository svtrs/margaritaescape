...
boolean mqttSimonStart = false;
boolean mqttSimonWin = false;
int mqttSimonDifficulty = 1;
boolean simonSolved = false;

boolean mqttStepPuzzleStart = false;
boolean mqttStepPuzzleSolve = false;
boolean spSolved = false;

boolean mqttPressureDiagnoseMode = false;
...
void callback(char* topic, byte *payload, unsigned int length) {
  ...
  Serial.println();
  if (strcmp(topic, mqttFullTopic("test")) == 0) {
    Serial.print("I see the topic");
  }
  if (!strcmp(topic, mqttFullTopic("Simon/")) & message == "solve") {
    mqttSimonWin = true;
  }
  else if (!strcmp(topic, mqttFullTopic("Simon/")) & message == "start") {
    mqttSimonStart = true;
  }
  else if (!strcmp(topic, mqttFullTopic("stepPuzzle/")) & message == "start") {
    mqttStepPuzzleStart = true;
  }
...
void loop() {
  ...
  else if (mqttStepPuzzleStart == true) {
    spSolved = false;
  }
...
void simonMode (int difficulty) {
  ...
  if (gameOver == 0 || mqttSimonWin == true) {                                          //checks if the game is won or not
    outwardRay(0, 255, 0);
    outwardRay(0, 255, 0);
    simonSolved = true;
    checkConnection();
...
void StepPuzzle(int playerCount) {                                                             //The first puzzle of the room, this function contains a parameter corresponding to the number of
  ...
  while (spSolved == false & mqttStepPuzzleStart == false) {
